...
const CryptoTokenTable = ({ data }: { data: any }) => (
  <Card className="bg-card border-border max-h-[350px] flex flex-col">
    <CardHeader className="flex-shrink-0">
      <CardTitle className="text-lg text-foreground flex items-center space-x-2">
        <Coins className="h-5 w-5 text-purple-500 dark:text-purple-400" />
        <span>Crypto Token Breakdown</span>
      </CardTitle>
      <CardDescription className="text-muted-foreground">
        Digital asset conversion statistics across different networks
      </CardDescription>
    </CardHeader>
    <CardContent className="flex-1 overflow-hidden p-0">
      {Object.keys(data).length === 0 ? (
        <div className="text-center py-8 text-muted-foreground px-6">
          No crypto token data available
        </div>
      ) : (
          ...
          <table className="w-full">
            ...
            <tbody>
              {Object.entries(data).map(
                ([token, stats]: [string, any], index) => {
                  const [network, currency] = token.split("_");
                  const growthValue = stats.weekly_growth || 0;
                  const hasPositiveGrowth = growthValue > 0;
                  const hasNegativeGrowth = growthValue < 0;
                      ...
                      <td className="py-4 px-4 text-right">
                        <div className="font-semibold text-foreground">
                          {stats.settled_amount
                            ? stats.settled_amount.toFixed(2)
                            : "0.000000"}
                        </div>
                      </td>
                        ...
                        {growthValue !== 0 && (
                          <Badge
                            className={`${
                              ...
                              hasPositiveGrowth ? "animate-pulse z-[1]" : ""
                            }`}
                          >
                            {hasPositiveGrowth && (
      ...
      <DashboardLayout>
        <div className="space-y-8 bg-background min-h-screen">
          ...
          <div className="grid gap-6 md:grid-cols-1 xl:grid-cols-2">
            ...
            <CryptoTokenTable data={hasError ? {} : cryptoBreakdown} />
          </div>
          {/* Quick Actions */}
          <Card className="bg-card border-border">
            <CardHeader>
