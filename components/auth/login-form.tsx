  ...
  useEffect(() => {
    ...
    if (sandboxParam === "true") {
      ...
      sonnerToast.info("Sandbox Mode", {
        description:
          "You're accessing ElementPay's sandbox environment for testing.",
        duration: 5000,
      });
    } else {
      switchToLive();
      sonnerToast.info("Live Mode", {
        description:
          "You're accessing ElementPay's live environment for production.",
        duration: 5000,
      });
    }
    ...
    try {
      ...
      if (result?.error) {
        ...
        if (
          ...
          result.error.includes("sandbox environment")
        ) {
          sonnerToast.error("Service Temporarily Unavailable", {
            description:
              "ElementPay authentication service is experiencing issues. Please try again in a few minutes.",
            duration: 10000,
            action: {
              label: "Retry",
              onClick: () =>
          ...
          sonnerToast.error("Server Error", {
            description:
              "The authentication service is currently down. We're working to resolve this issue.",
            duration: 10000,
            action: {
              label: "Contact Support",
              onClick: () =>
                window.open(
                  "mailto:support@elementpay.net?subject=Login%20Issues",
                  "_blank"
                ),
            },
          });
      ...
      {isSandbox && (
        <div className="bg-blue-50 border border-blue-200 text-center rounded-lg p-3 mb-4">
          <p className="text-xs text-blue-600">
            You're logging into ElementPay's sandbox console for testing and
            development.
          </p>
        </div>
      )}
